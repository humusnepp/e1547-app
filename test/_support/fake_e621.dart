...
class FakeE621 {
  ...
  late final Router _router = Router()
    ...
    ..get('/posts/<id|[0-9]+>/similar/<kind>.json', _listSimilar)
  ...
  Future<Response> _route(Request request) async {
    ...
    if (state.errorPaths.contains('/${request.url.path}')) {
      return _json({'message': 'Internal error'}, status: 500);
    }
    try {
  ...
  /// Mirrors the reduced post payload of `/posts/<id>/similar/<kind>.json`.
  Response _listSimilar(Request request, String id, String kind) {
    if (kind != 'tags' && kind != 'artist') {
      throw FakeE621Error(404, 'Not found');
    }
    final postId = int.parse(id);
    final others = state.posts
        .where((e) => e['id'] != postId)
        .toList();
    return _json({
      'post_id': postId,
      ...
      'results': [
        for (var index = others.length - 1; index >= 0; index--)
          {
            'post_id': others[index]['id'],
            'score': index + 1,
            'explanation': null,
          },
      ],
      'post_data': others.map(_similarPost).toList(),
    });
  }

  Map<String, Object?> _similarPost(Map<String, Object?> post) {
    final file = post['file']! as Map<String, Object?>;
    final preview = post['preview']! as Map<String, Object?>;
    final sample = post['sample']! as Map<String, Object?>;
    final score = post['score']! as Map<String, Object?>;
    return {
      'id': post['id'],
      ...
      'flags': post['flags'],
      'pools': (post['pools'] as List?)?.join(' '),
      'rating': post['rating'],
      'tags': (post['tags']! as Map<String, Object?>)
          .values
...
class FakeRequest {
  ...
  @override
  String toString() => '$method $path $headers $query $body';
}
...
class FakeE621State {
  ...
  factory FakeE621State.empty() => FakeE621State(
    posts: [],
    favorites: {},
    topics: [],
    replies: [],
  ...
  /// Paths which the fake answers with `500`, to simulate server errors.

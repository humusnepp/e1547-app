...
class PostClient {
  ...
  /// Fetches posts which are similar to a post.
  ///
  /// Similarity is either computed by shared [PostSimilarKind.tags] or by
  /// sharing an artist with the source post.
  Future<PostSimilar> similar({
    required int id,
    required PostSimilarKind kind,
    CancelToken? cancelToken,
  }) =>
      dio
          .get(
            '/posts/$id/similar/${kind.name}.json',
            cancelToken: cancelToken,
          )
          .then((response) => PostSimilar.fromJson(kind, response.data));
